...
const AttendanceAI = () => {
    ...
    const [loading, setLoading] = useState(false); // 사원 목록(근태) 로딩
    const [aiLoading, setAiLoading] = useState(false);

    const [error, setError] = useState(null); // 근태 목록 에러
    ...
    // selectedEmpId가 생겼는데 report가 비어있으면 자동으로 1회 가져오기
    useEffect(() => {
        if (selectedEmpId && !report) {
            fetchEmployeeReport(selectedEmpId);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [selectedEmpId]);
    ...
    // records에서 unique empId 옵션 구성
    const employeeOptions = Array.from(
        new Map(records.map((r) => [r.empId, r])).values()
    );
                                    ...
                                    <div className="att-ai__chart-block">
                                        <Line
                                            ...
                                            options={{
                                                responsive: true,
                                                plugins: {
                                                    title: { display: true, text: "일자별 근무 분(분)" },
                                                },
                                            }}
                                        />
                                    </div>
